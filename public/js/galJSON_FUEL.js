        ...
        success: function (data) {
           ...
           /*  var output = '<label>Vehicle Name: </label> <small style="color:grey; margin-left:10px">(format : AKD 736 MU)</small>'
            ...
            /*   output += '<select class="form-control select2" style="width: 100%;" id="vehicleName" name="vehicleName">';
                 ...
                 // var stringifyResponse = JSON.parse(data.CommonResult.DataSet);
                 //shuffleArray
                 var myShuffuledArray = data.CommonResult.DataSet.sort();
                   //console.log(myShuffuledArray);
                for (var idx = 0; idx < data.CommonResult.DataSet.length; ++idx) {
                    
                    output += '<option value="'+ myShuffuledArray[idx][12] + '-'+ myShuffuledArray[idx][11] + '-'+ myShuffuledArray[idx][5] + ' ">'+ myShuffuledArray[idx][12] + '</option>';
                }
                
                
                output += '</select>';
                  output += '<center><br/><input type="hidden" name="dType" id="dType" value="fueling" /><button hidden id="synDepreciation" type="submit" onClick="process()" class="btn btn-primary btn-fill btn-xs">Process</button></center<br/>\n\
                    </form>';
      ...
      $.ajax({
        ...
        success: function (data) {
             $('#fromgalooli').html('Processing Data, please wait....');
                ...
                if(data.status == 200){
                   setTimeout(function(){window.top.location= GLOBALS.appRoot + "form/fuel_requests?vehicle_number="+data.vehicle_name+"&mileage_km="+data.milleage_km+"&fuel_level="+data.fuel_level+""} , 100);
                 }
            }
         }).fail(function () {
